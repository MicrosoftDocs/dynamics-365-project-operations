...
Microsoft Dynamics 365 Project Operations uses a unique security model that works with Microsoft Office Groups to provide a role-based business security model.
...
| Project system | Role used for application   context. Customers shouldn't use this system role.                                    | Global |

## Security enforcement

Actions that are performed at the project level are performed in the context of the logged in user. This means that in order to create, open, or delete a project, the user is required to have access available in CDS. Access in CDS might be granted through any of the possible mechanisms included in the platform. For example, a user with a larger scope might access the project or if an explicit project share action has been performed which grants the user access.
...
Project Operations doesn't create a group through implicit action and only does so through the explicit action of pressing groups.
...
:::image type="content" source="./media/groupsmode.png" alt-text="Screenshot of group mode diagram showing the group assignment process.":::

1. The Project is created and owned by the creating User.
1. The Project owner is updated to the team.
1. The Owner team is mapped to the specified or created Office Group.
1. The original owner of the Project is added to the Office Group.
