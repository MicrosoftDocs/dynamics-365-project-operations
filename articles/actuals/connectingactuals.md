...
Transaction connection is an entity that stores the relationship between two related business transactions, such as cost and related sales actuals, or reversals triggered by billing activities like invoice confirmation or invoice corrections.
...
- The existing text fields (record ID and record type stored as text) remain and aren't removed.
...
Each event in the processing workflow triggers the creation of records in the Transaction connection table. These records capture the relationship between the two participating records by using both:

- Existing text-based identifiers (record ID and type).
- Polymorphic lookup references (direct record links).

These records help build an efficient trace of the relationships across time entry, journal line, actual, and invoice line details.

The following table shows the records in the **Transaction connection** entity for the preceding workflow.

> [!NOTE]
> If you're processing or navigating connections programmatically, prefer the **polymorphic lookup** fields on the transaction connection record. The **Transaction 1 type / Transaction 2 type** columns in this table describe the logical record type that the relationship points to and correspond to the record types used by the text-based fields.
