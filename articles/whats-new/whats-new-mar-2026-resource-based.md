...
- Project Operations in a Microsoft Dataverse environment version 4.165.3409.5.
...
| --- | --- | --- |
| Approvals |**New Approvals Experience.** <br><br> The UI for approvals is updated to a form with tabs for each type of approval record. By using this new UI, approvers can quickly see how many records of each type are pending their approval. This new UI also uses the time phased grid for time approvals, but is customizable to user's preferences.| |
...
| Project Budgeting |**Enable improved project budgeting time phasing grid.** <br><br> With the improved Project Budget time‑phased grid, budget line details now display on a single row. Budget periods are distributed across the same row based on the configured budget periods. This change provides a clearer, more intuitive, and easier‑to‑analyze view of time‑phased budgets. All existing budget actions and states are fully supported. The feature is available behind the **Enable improved project budgeting time phasing grid** feature flag.| [Use time-phased project cost budget lines](../pro/budget/time-phase-budget-line.md) |
| Project Financials |**Enable editing of due date for project invoices.** <br><br> You can manually edit the due date on project invoice proposals, providing flexibility to align payment terms with customer agreements independent of automatically calculated dates.| [Defining customer due date](../prod-pma/project-invoicing.md#defining-the-customer-due-date)|
| Project Financials |**Enable selective import from staging for invoice proposals.** <br><br> This feature streamlines population of project invoice proposal lines when you don't want to wait for the scheduled staging process to run. A new **Import lines** button on the project invoice proposals screen allows selective import rather than a full batch run.|[Import from staging](../invoicing/format-update-project-invoice-proposals.md#selective-import-from-staging) |
| Project Financials |**Use the modern architecture for existing legal entities with project data.** <br><br> More scenarios were added to this feature to support sales orders, item requirements, and sales agreements in existing PMA projects.| [Move to the modern architecture](../prod-pma/move-to-modern-architecture.md) |
| Sales |**What-if Analysis on Quotes.** <br><br> What‑if Analysis in Project Operations lets you model different staffing and pricing strategies, view real-time financial KPIs, and compare scenarios side by side—helping you apply the most profitable option directly to your project quote.| |
| Subcontracting |**Enabled Subcontracting UX improvements.** <br><br> A new **Hierarchy** view is available on the **Subcontracts list** page, enabling easier navigation across related subcontract data. This view presents Subcontracts, their associated subcontract lines, and the linked project in a clear hierarchical structure, making it simpler to understand relationships and move between related records. This enhancement is available behind the feature flag **Enable Subcontracting UX enhancements for smoother navigation and insights**.| [Enable improved subcontracting UX](../pro/subcontracting/improved-sub-contracting-ui-experience.md) |
...
| Approvals | 6114170 | Export to Excel isn't working for project approval entity after selecting the records. |
...
| Subcontracting| 6000962 | Subcontract field isn't defaulted on vendor invoice lines based upon subcontract line. |
| Time Entry | 6100760 | Time off entries don't appear on work hours calender when time entry of vacation or absence type is created on a working day. |
