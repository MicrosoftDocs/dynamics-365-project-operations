...
- Project Operations in a Microsoft Dataverse environment version 4.169.3495.2.
...
| Time Entry | **Configure maximum hours that can be logged in a day** <br><br> This feature is part of a broader set of time entry behaviors that you can now configure on an environment. Admins can now configure the maximum number of hours for time entries that a user can log in a day (currently set to 4 hours out of the box). | [Time Entry Configurations](../time/time-entry-configurations.md) |
| Time Entry | **Configure time zone behavior on the time entry grid** <br><br> This feature is part of a broader set of time entry behaviors that admins can now configure on an environment. Admins can now switch the time zone behavior from time-zone aware (default, out of the box) to time-zone independent to prevent time zone related discrepancies for global teams. | [Time Entry Configurations](../time/time-entry-configurations.md) |
...
| Project Financials | **Post project invoice proposals using multithreaded batch tasks** <br><br> This feature provides enhanced performance for late selection. With late selection, a batch job evaluates its query criteria at run time instead of at submission time. For recurring invoice posting jobs, this feature means the job automatically includes proposals created after the job was scheduled, without any manual intervention. Posting occurs using multiple batch tasks to split the work. | [Manage project invoice proposals](../invoicing/format-update-project-invoice-proposals.md#batch-posting-invoice-proposals-with-late-selection) |
