...
Project Operations offers robust capabilities for capturing cost and revenue associated with project activities. Project postings are the financial results driven from the applicable business processes. These results are driven by a combination of:
...
Whether postings will impact the profit and loss (P&L) or balance sheet (WIP) on your financials
When revenue is invoiced vs. accrued
...
You can categorize revenue and expenses on projects to control posting to the general ledger, and for detailed reporting and analysis. By categorizing expenses and revenues separately from the general ledger, project managers can get additional details about project performance.
...
Project categories are the basic grouping for project transactions. Individual project categories can have unique cost and revenue accounts and be used to differentiate between travel exepenses related to hotels, and travel expenses related to mileage. When you enter a project category on a transaction line, the category selection is limited to the categories that are set up and linked to this transaction type. For example, when an expense transaction is entered, only expense categories are available. You can set up categories by going to Project management and accounting > Setup > Categories > Project categories.

Shared categories is a parent grouping of project categories that can be shared across applications and modules. Microsoft Dynamics 365 uses the shared categories concept to categorize expenses in different applications, such as Microsoft Dynamics 365 Finance, Microsoft Dynamics 365 Supply Chain, and Project Operations. A shared category must exist before a project category can be created in any consuming legal entity.  You can review and adjust the shared categories by going to Project management and accounting > Setup > Categories > Shared Categories.
...
## Project Groups
Your project may be an internal project tracking costs only, or it could be a revenue generating project. The different project types are discussed more in [Create Projects]https://learn.microsoft.com/en-us/dynamics365/project-operations/prod-pma/overview-project-management-accounting. Project groups provide the ability to determine many of the fundamental accounting rules of how the project financials should be generated. For hour, expense, item, and on-account transactions, you can choose to post transactions to either Profit and loss or Balance accounts. In certain cases, you can also choose not to post hour transactions.
...
Non‑chargeable – Not billable, no revenue, usually direct P&L
Internal – No revenue, but costs may be capitalized (e.g., investment projects)
...
3. Select main accounts. Evaluate posting rules using ledger posting setup using:
...
4. Apply line property rules

Chargeable = sales price and invoicing behavior
Accrue revenue = use accrued revenue accounts instead of (or ahead of) invoiced revenue accounts
Capitalize cost = post to WIP accounts

5. Create ledger entries

For each posting type (Cost, WIP cost, Revenue, Accrued revenue, Invoiced revenue, etc.), debit and credit accounts are derived from the above configuration. 