...
You are working on a Web site design project for a customer and charging a flat fee of USD 10,000. You forecast that it will take 100 hours (USD 5,000) to complete the project. You also forecast two plane tickets and four nights in hotels for trips to the customer's site (USD 1,800). This results in a total forecasted cost of USD 6,800.

When you run the fixed-price revenue recognition process to create an estimate at the end of the month, you find that you have worked 35 hours on the project to this point. This doesn't include flights or hotel stays yet. You also had an assistant perform five hours of research for the project at a cost of USD 100, which you hadn't planned for.
...
2. Select **New** to create new cost template and enter a name and description.
...
4. Select a default completion method:
  ...
  > [!NOTE]
  > For manual calculations, the percentage of completion is maintained in the **Manual calculation** field on the **General** tab on the **Estimate** page.

5. In the **Completion based on** field, select a value:

  - **Amount**: The amount in accounting currency is used when calculating the percentage of completion.
  - **Unit**: The quantity is used when calculating the percentage of completion.
...
7. On **Categories** tab, select the project categories to be included on the cost template line.
8. On **General** tab, select if this line will be included in the percentage of completion calculation.
