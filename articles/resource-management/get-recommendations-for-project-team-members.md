...
description: Learn how to get optimal recommendations for project team members before booking them. Follow step-by-step instructions to find the best fit for your project.
author: mohitmenon
ms.date: 02/05/2026
ms.topic: how-to
ms.custom: 
...
1. Go to **Projects**, and select a project that you need to staff with one or more resources.
...
Each suggested resource is assigned to a **recommendation score**, which is a number out of 10. This score indicates the resource's appropriateness for the specific team member requirement. It's based on a combination of the following metrics (factors): **Experience Fit**, **Skill-match**, **Cost Impact**, and **Availability**. The following list explains the relevant factors and sections:
...
You can configure which resourcing units and factors are considered, along with their relative importance when suggesting resources.

- **Resourcing units to consider:** By default, the system considers only the resourcing unit mentioned in the team member requirement and uses it to provide suggestions. However, you can modify the resourcing units by selecting one or more units in the **Show resources from** dropdown list.

    After you modify the resourcing units, select anywhere outside the dropdown list to update the suggestions. The updated suggestions include resources across the selected units.

- **Factors to consider:** The settings symbol in the upper right of the side pane lets you modify default settings and include or exclude factors to arrive at the list of suggested resources. Assign a relative importance (**High**, **Medium**, or **Low**) to each included factor for scoring all suggested resources.

  - By default, all factors are included and set to **Medium** importance, unless a project is over budget or running behind schedule. If a project is over budget, the **Cost Impact** factor is set to **High** importance. If a project is running behind schedule, both the **Experience Fit** factor and the **Availability** factor are set to **High** importance.
