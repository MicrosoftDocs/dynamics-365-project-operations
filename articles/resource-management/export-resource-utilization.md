...
4. On the toolbar, select the **Export to Excel** icon.
   ...
   The system begins preapring the data for download. The export includes utilization data for all resources that match your current filters.

   > [!IMPORTANT]
   > You can run only one export at a time. Wait for the current export to complete before starting another.

5. When the data is prepared, you are notified that the file is ready to download.
...
| **Non Chargeable Time Actuals** | Hours logged as non-chargeable time actuals. |
...
| **Non Billable Utilization** | Percentage of capacity used for non-billable work. |
