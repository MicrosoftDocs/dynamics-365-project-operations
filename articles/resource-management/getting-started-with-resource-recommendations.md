...
description: Learn how to enable and configure the resource recommendations feature in Microsoft Dynamics 365 Project Operations. Follow these prerequisites to get started.
author: mohitmenon
ms.date: 02/05/2026
ms.topic: how-to
...
1. Select the **My Dataflows** tab. This tab lists the following dataflows (in addition to any pre-existing items):
...
1. Select **Dataflows** from the list of **Objects** on the left. You see a list of all Dataflows.
1. Find the _Owner_ for the Dataflows you need in this feature.
1. If the owner isn't a _System User_, go back to **Home** and select **Dataflows** from the left navigation pane.
...
1. Go to **All Dataflows** and search for the name of the Owner you found in the previous step.
...
1. When you complete these steps for all four dataflows, you see them in **My Dataflows** and you can continue to activate each one of them.
...
1. Select the **EnvironmentName** parameter. Then, in the space provided for **Current Value**, replace the placeholder text (_\{\{envName\}\}_) with the URL of the environment you're using, **without the "https://"** (for example, enter _recommend.crm.dynamics.com_).
