...
The approvals agent in Project Operations assists project managers by completing an initial review of all time, expense, and material entries submitted by using a combination of business logic and LLM to evaluate each entry against the organization’s policy documents. 
  Based on the agent’s interpretation of the relevant policy document and review outcomes, if there are no policy violations, the agent determines it to be low risk and classifies it as ready for approval. If there are one or more policy violations, the agent determines it to be high risk and flags these for additional human review, providing a rationale for why additional review is needed. 
...
- Provides a first review of all submitted time and expense approvals and suggests which ones need additional review.
...
### What is/are Approvals agent’s intended use(s)?
Intended uses of the agent are:
- To help classify transactions as high-risk or low-risk transactions and to assign a status of “ready for approval” to transactions that do not have policy violations. The agent assigns a status of “needs review” to transactions that have 1 or more policy violations. 
...
- In addition to this, the content generated by this system was evaluated through a red-teaming process to test for groundedness and various kinds of harms. 
...
- The accuracy of the output will vary greatly depending upon the quality of the input, i.e., the policy documents, so the documents must be crafted with care. The system may not perform well if approval criteria in the policy documents are ambiguous or if there are conflicting policies in the policy documents.
...
- **To mitigate the above issues**:
    ...
    - In certain cases, users may need to tweak the language used in the policy documents to improve accuracy of output (i.e. the review status and reason generated by the agent). 
    - Users should avoid creating conflicting policies in the documents to avoid confusing the agent. 
    - While currently 2 modes of classification are supported, we suggest using Classify Only mode during configuration to prevent automatic approval of “ready for approval” transactions. 
...
- At any point in time, if any issues are observed with the system output, admins will have the flexibility to disable the feature using feature control settings.
- If the feature is enabled, only record types with an uploaded policy document will be reviewed. For example, if only a time policy document is uploaded, then only time entries will be reviewed by the approvals agent. - - If no policy documents are uploaded, then no records will be reviewed. 
...
- During the production ready preview of the Approvals agent, there will be a mechanism for users to provide thumbs up/down feedback on any reviewed record. 
