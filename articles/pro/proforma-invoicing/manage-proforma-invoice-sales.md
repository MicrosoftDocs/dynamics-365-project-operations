...
| **Select Product** | Set by default from the source actual, this is read-only field. | When you create a new invoice line detail without a backing actual, this field can be edited. |
| **Product** | Set by default from the source actual, this is a read-only field. | When you creat a new invoice line detail without a backing actual, this field can be edited if the **Select Product** field is set to **Exsting product**. |
| **Product Name** | Set by default from the source actual, this is a read-only field. | On a new invoice line detail, where the product ID is seelect from catalog, this field is set to the product name. For a write in product, the field is set to the write in name. |
| **Write In Description** | Set by default from the source actual, this field is read-only. | When you create a new invoice line detail withough a backing actual, you can add a write in description for the product. |
