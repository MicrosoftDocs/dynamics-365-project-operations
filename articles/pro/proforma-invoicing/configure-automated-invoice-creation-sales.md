...
You can configure automatic invoice creation in Dynamics 365 Project Operations. The system creates a draft proforma invoice based on the invoice schedule for each project contract and contract line. You configure invoice schedules at the contract line level. Each line on a contract can have a distinct invoice schedule, or you can include the same invoice schedule on every line of the contract.

When you create an invoice, the system always creates at least one invoice per project contract. In some cases, the system creates multiple invoices. For example, if the contract has multiple customers, the system creates the same number of invoices as the number of customers that have billable transactions to invoice on that project contract.
...
1. In the **Job Type** field, select **None**. By default, the **Frequency Daily** and **Is Active** fields are set to **Yes**.
...
The batch process job for creating invoices is a recurrent job. If you run this batch process many times, you create multiple instances of the job and cause errors. Therefore, start the batch process only one time, and then restart it only if it stops running.
