...
2. Enter the required information, and then select **Save**.
    ...
    | Status Reason | The status of the subcontract. The supported values are:</br>• Draft </br>• Confirmed</br>• Closed </br>• Canceled</br>| The status determines whether the subcontract can be edited. Only Subcontracts in status of **Draft** status can be edited. **Confirmed** status on a subcontract should be used to indicate that the negotiation with vendor is complete and the subcontract is approved for delivery. **Closed** status on a subcontract should be used to indicate that the delviery on the subcontract is complete. **Canceled** status on a subcontract should be used to indicate that the subcontract was canceled and that no delivery is anticipated on that subcontract anymore.  | 
