...
description: Learn how to block vendor payments in Microsoft Dynamics 365 Project Operations until approved by a project manager. Step-by-step setup and process guide included.
author: mukumarm
ms.date: 02/04/2026
ms.topic: how-to
ms.custom: 
...
This article describes the features in Microsoft Dynamics 365 Project Operations that facilitate withholding vendor payments for vendor invoices until they receive approval from the project manager in Dataverse. In some cases, project manager authorization is required before vendor payment can be initiated. In these cases, accounts payable (AP) clerks document the invoice and wait for the project manager's approval before they process the payment.

To use the functionality described in this article, you must enable the **Enable Hold Vendor Payment for vendor invoices till PM confirms the invoice for resource based/non-stocked scenarios** feature in the **Feature management** workspace in Dynamics 365 Finance.
...
:::image type="content" source="../media/BlockvendorPaymentDualWrite.png" alt-text="Screenshot of the dual-write entity map for vendor invoices.":::

The following screenshot shows the dual-write field mapping that's used for vendor invoices.

:::image type="content" source="../media/VendorInvoiceDualwritefieldmapping.jpg" alt-text="Screenshot of the dual-write field mapping for vendor invoices.":::
...
1. On the **Financial** tab, if the project manager must verify the vendor invoice in Dataverse, set the **Manual verification by PM is required** option to **Yes**. The **Block vendor payments till PM confirmation** field becomes available.
...
When an AP clerk receives an invoice from the subcontractor, they create a new invoice in Finance.
...
1. Select the procurement category that you created for the subcontract lines, and enter the unit price, unit of measurement, and quantity.
...
When you post the vendor invoice, it becomes available in Dataverse for project manager verification and processing. For information about verification of vendor invoices in Dataverse, see [Confirm project vendor invoices](../../procurement/confirm-a-project-vendor-invoice.md).
