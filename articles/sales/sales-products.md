...
Products are the backbone of your business. The product catalog in Dynamics 365 Sales Professional is a collection of products and pricing information. Make it easier for your sales reps to increase their sales by quickly creating a product catalog.
...
1. In the site map, under **Setup**, select **Products**.
1. Select **Add Product** and fill in the following information:

    - **Name**
    - **Product ID**
    - **Parent**: Select a parent product family for the product. If you're creating a child product in a product family, the name of the parent product family is populated here. You can't change this value after you save the record.
...
1. Select **Save**.
1. On the **Additional Details** tab, in the **Price List Items** section, select **More commands**, and then select **Add New Price List Item**.
1. On the **Additional Details** tab, in the **Product Relationship** section, select the **More commands** icon, and then select **Add New Product Relationship.**
1. In the **New Product Relationship** form, enter the following details, and on the command bar, select **Save and Close**:
    ...
    - **Direction**:Select whether the relationship between the products is unidirectional or bidirectional. When you select unidirectional, the product that you select in **Related Product** is shown as a recommendation for the existing product but not vice versa.

1. On the Product form, select **Save**.

## Import products

Use import templates to bring bulk product data into Dynamics 365 Sales.
...
1. In the site map, select **Products**.
1. Open an active product that you want to change, and on the command bar, select **Revise**.
1. In the **Confirm Revise** dialog box, select **Confirm**. This action changes the product status to **Under Revision**.
1. After you're done making changes, on the command bar, select **Publish**.

    > [!TIP]
    > To revert the changes and continue with the last active version of the product, select **Revert**. This action changes the status of the product back to **Active**.

## Clone a product

When you're creating a new product, save time by cloning an existing one. This process creates a copy of the original record with all the details except for the name and ID.

1. Make sure that you have one of the following security roles or equivalent permissions: System Administrator, System Customizer, Sales Manager, Vice President of Sales, Vice President of Marketing, or CEO-Business Manager.
1. In the site map, select **Products**.
1. Select a product record that you want to clone, and on the command bar, select **Clone**. A confirmation dialog box appears.
1. Select **Confirm**.
...
If your organization no longer sells a product, retire it so that your sales agents can't access the product.

1. Make sure that you have the System Administrator or Sales Professional Manager role or equivalent permissions.
1. In the site map, select **Products**.
1. Open an active product that you want to retire, and on the command bar, select **Retire**.
...
1. Make sure that you have the System Administrator or Sales Professional Manager role or equivalent permissions.
1. In the site map, select **Products**.
1. Select a product record you want to delete, and on the command bar, select **Delete**.
1. In the **Confirm Deletion** dialog box, select **Continue**.

## Quantity factors for products

Quantity factors support the sale of subscription-based products. For subscription-based products, express the quantity on the quote or project contract line as the number of user months.

Usually, you store the price of subscription software in the catalog as the price per user per month. However, you can use other time descriptions instead. During the sales process, the price on the quote line is usually the per-user, per-month price that the IT sales agent negotiates and discounts. Each deal has a different number of users and a different number of subscription months. The quantity that you use to compute the amount of the quote line is a product of the number of users and the number of subscription months.
...
The system validates that only numeric properties or product properties that have a numeric data type are flagged as quantity factors. When you add a product with quantity factors to a quote line, the **Quantity** field on the quote line becomes a read-only field. After you enter values for product properties that are quantity factors, the quantity of the quote line is calculated.
