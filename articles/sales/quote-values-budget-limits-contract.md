...
| Potential Customer | Summary tab | Reference to the customer's company or account record. A valid customer to reference on the project quote must be set up as a customer in the owning company of the quote. The owning company shows the list of legal entities and these are set up in the **Project management and accounting** module of Project Operations. When a quote is created from an opportunity, this field is copied from the corresponding field on the opportunity. | The currency on the project quote is defaulted based on the currency of the customer. This can, however, be changed before the quote is saved. |
