...
You can add an invoice schedule to a project-based contract line. You can only invoice after the contract is won and you're creating a project contract. An invoice schedule automatically creates draft invoices for a project-based contract line. However, if you only manually create invoices, you can skip the creation of invoice schedules on contract lines.

## Create a time and material invoice schedule for a contract line

When a project-based contract line uses the time and material billing method, you can create a date-based invoice schedule. To automatically generate a date-based invoice schedule, complete the following steps.
...
1. On the subgrid, select **Generate Invoice Schedule**. The invoice schedule is generated, and the **Invoice Run Date**, **Transaction Cutoff Date**, and **Run Status** fields are set in the following way:
    ...
    - **Run Status**: Automatically set to **Not Run**. When the automatic invoice creation job runs for a certain invoice run date, this field updates to **Run Successful** or **Run Failed**.
...
> [!NOTE]
> You can create milestones only on contract lines that a project is mapped to.
...
1. On the subgrid, select **Generate Periodic Milestones**. The invoice schedule is generated, and the **Milestone Name**, **Milestone Date**, and **Milestone Amount** fields are set in the following way:
    ...
    If the contract line has a value in the **Estimated Tax amount** field, the process apportions this field equally to each milestone when generating periodic milestones.

Billing milestones must equal the contracted value of the contract line. Otherwise, you receive an error on the **Contract Line** page. You can fix the error by creating, editing, or deleting milestones to ensure that the total billing milestones equal the contracted value of the line. After you make the changes, refresh the page to remove the error.
...
1. Open the fixed price contract line that you're creating a milestone for. On the **Invoice Schedule** tab, select **Create new Contract line milestone** on the subgrid. 
1. On the **Milestone Creation** page, enter the required information, based on the following table.
    ...
    | Invoice Status | Quick Create | When a milestone is created, set this status to **Not Ready for Invoicing** or **Not Started**. | This field is carried over to the project contract line milestone and the invoice. |
    | Line Amount | Quick Create | The milestone amount or value that you invoice to the customer. | This field is carried over to the project contract line milestone and the invoice. |
    | Tax | Quick Create | The tax amount that you apply on the milestone. | This field is carried over to the project contract line milestone and the invoice. |

1. Select **Save and Close**.

## Create a progress billing invoice schedule for a contract line

*Progress billing* lets you invoice customers based on the percentage of work that you complete instead of fixed milestones or time and materials. Progress billing is useful in long-term projects or contracts where the work spans months or years.
