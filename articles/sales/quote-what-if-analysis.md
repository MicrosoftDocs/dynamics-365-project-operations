...
What-if analysis on a project quote enables you to simulate adjustments to **work quantities and prices on quote line details** so you can immediately see how those changes affect the quote's key financial metrics. The changes don't alter the actual quote until you choose to apply a scenario.
...
- What happens to margin if you shift more hours to a lower-cost delivery center?
- How does a 10 percent rate increase for a specific role affect overall profitability?

For each scenario, the system recalculates and displays four key financial metrics (KPIs) along with the difference from the baseline quote. This process provides real-time visibility into the financial impact of each change.
...
- Activated or closed quotes are read-only and can't be used for what-if analysis.
- You can create multiple what-if scenarios for the same draft quote.
- If you modify the quote, existing scenarios become invalid and you must recreate them.

To create a what-if scenario, follow these steps:

1. Open the quote and go to the **What-if analysis** tab. This tab appears when the What-if Analysis preview feature is enabled through the feature flag in your environment.
1. Select **Create scenario** to create a new scenario.
1. Enter a title for the scenario and optionally add a description. The quote for which you create the scenario is automatically selected. By default, the Time transaction class is selected, meaning you adjust quantities or prices for quote line details that use the Time transaction class.
...
As long as the quote remains in Draft status, you can create new what-if scenarios to explore other adjustments by using the now-updated quote as the new baseline.
