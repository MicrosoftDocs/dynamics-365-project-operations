...
The **Troubleshoot** tab shows a list of error messages that users encountered during the **Import from staging** or **Project integration journal** posting processes. This tab helps you identify the root cause of any logged issues, so that you can make the necessary corrections. Previously, any error messages encountered in the **Project integration journal** were not stored anywhere other than in the **Action Center** and **Infolog**. Once the feature is enabled, errors are logged into a permanent table so they can be reviewed, filtered, grouped, and better analyzed. 

Note:  Only a subset of errors are logged to the worksapce at this time. Read further for additional details on error messages that are included. This list of errors and covered scenarios will grow over time.
...
| No open fiscal period was found in or after the specified accounting date 11/20/2024. | The specified fiscal period was not open for posting the integration journal and no future period was open to move the transaction into.   | In **Ledger calendars**, open the specified period or a future period for posting and try posting again |
