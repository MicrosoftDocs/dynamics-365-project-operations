...
1. In the dialog that appears, select **Create** to create a connection to Copilot Studio, and sign in as the user that you want the agent to run as.
    ...
    > [!NOTE]
    > The user who signs in for these connection will by fthe user the agent acts as. This must be a real user with an Entra ID account (sign in must happen via OAuth and not as a service principal) 
