...
When you specify a project for the journal, a new **Project** tab becomes available.

Enter an amount into the debit or credit column to determine the cost price for the journal. A **Description** is optional and flows through to Dataverse onto the customer invoice. You can modify the currency for cost from the defaulted company currency. Once you enter the account, offset, and price, select the **Project** tab to complete the remaining required details.

On the **Project** tab, consider the following fields when creating an entry:
...
General journals aren't aware of Dataverse pricing. The journals allow manual entry of costs and sales. You might see the warning "**Sales price not specified for time and material projects.**"

When you post the transaction, cost commits immediately to general ledger and the project subledger. This process doesn't use the expense integration account and instead, unlike expense reports, posts immediately. A connection is made to Dataverse by using the **IOrganization** service, and creates a journal header and journal lines for each cost and sales amount in general journal. 
...
- Using a cost currency that's different from the project currency requires the UR60 release of Project Operations. Without this update installed, the currency might be posted differently in the Dataverse journal from what was posted in the PMA journal.
- The client limits journals to 200 project-related lines. Previously, this limitation was at 50 lines as of 10.0.45. For optimal performance of synchronization and availability in Dataverse, like during period end close, use smaller journals to post if possible.
...
- You can't use both modern projects and Project Management and Accounting (PMA) projects in the same journal when customers have previous projects and are [transitioning to using the modern architecture](../prod-pma/move-to-modern-architecture.md).
- If you post multiple journals concurrently, avoid using the same projects in multiple journals when possible. Limit journal size to avoid table locking on Dataverse entities where the same project updates cause aggregate totals to be updated and cause locks. The retry logic eventually handles this scenario, but it can introduce delay in confirming the journal and generating Dataverse actuals.
- The process automation slightly lags in knowing if the journal is confirmed in Dataverse or not. Check for a confirmed journal or the existence of the actuals from the journal in Dataverse to know if the transaction is fully synced or not. The integration tracking in Dynamics 365 Finance only checks for updates on posted status at the start of every process automation recurrence.
